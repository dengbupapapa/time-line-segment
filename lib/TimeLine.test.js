                ...
                _createClass(TimeLine, [{
                    ...
                    value: function execute() {
                        ...
                        this._executeForPrevFinishCallback = function () {
                            //如果是暂停的就切换到对应segment后执行完就暂停
                            if (isPaused) {
                                if (segment.hasEventListener(FINISH, _this7._executeForTargetSegmentFinishCallback)) {
                                    segment.removeEventListener(FINISH, _this7._executeForTargetSegmentFinishCallback);
                                }
                                _this7._executeForTargetSegmentFinishCallback = function () {
                                    _this7.pause();
                                    _this7._executeing = false;
                                    segment.removeEventListener(FINISH, _this7._executeForTargetSegmentFinishCallback);
                                    _this7._executeForTargetSegmentFinishCallback = undefined;
                                };
                                segment.addEventListener(FINISH, _this7._executeForTargetSegmentFinishCallback);
                            } else {
                                _this7._executeing = false;
                            }
